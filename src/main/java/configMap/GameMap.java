...
public class GameMap {
  ...
  private int typeE(Enemy e) {
    if (e instanceof Dreth) return 1;
    if (e instanceof Fyron) return 2;
    if (e instanceof Gazer) return 3;
    if (e instanceof Kryon) return 4;
    if (e instanceof Liche) return 5; else return 6;
  }
  ...
  public ArrayList<Enemy> listOfAllEnemies() {
    ...
    for (int i = 0; i < tiles.length; i++) {
      for (int j = 0; j < tiles[0].length; j++) {
        if (tiles[i][j].getElem() instanceof Enemy) list.add(
          (Enemy) tiles[i][j].getElem()
        );
      }
    }
    return list;
  ...
  public ArrayList<Tower> listOfAllTowers() {
    ...
    for (int i = 0; i < tiles.length; i++) {
      for (int j = 0; j < tiles[0].length; j++) {
        if (tiles[i][j].getElem() instanceof Tower) list.add(
          (Tower) tiles[i][j].getElem()
        );
      }
    }
